...
class Avatar3D(nn.Module):
    def __init__(self):
        super(Avatar3D, self).__init__()
        self.resnet = load_ResNet50Model()
        self.mlp = MLP()
    
    def forward(self, image, keypoints):
        x = self.resnet(image)
        x = torch.cat((x, keypoints),1)
        x = self.mlp(x)
        return x


class MLP(nn.Module):
    def __init__(self, channels = [2048 + 137*3, 1024, 21*3], last_op = None):
